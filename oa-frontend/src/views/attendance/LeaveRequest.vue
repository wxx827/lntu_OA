  ...
  <div class="fade-in">
    ...
    <div class="page-header">
      <div class="header-left">
        <h2>请假申请 <span class="subtitle">Leave Application</span></h2>
        <p class="desc">提交和管理您的请假申请，查看审批进度</p>
      </div>
    ...
    <el-card shadow="never" class="card-shadow main-card">
      ...
      <div class="table-toolbar">
        <el-input 
          ...
          clearable
        />
        <el-select v-model="filterType" placeholder="请假类型" style="width: 140px; margin-left: 12px" clearable>
      ...
      <el-table :data="filteredRequests" style="width: 100%; flex: 1" height="100%" :header-cell-style="{ background: '#f5f7fa' }" stripe>
        <el-table-column prop="leaveType" label="请假类型" width="120">
          <template #default="{ row }">
            <el-tag :type="getLeaveTypeTag(row.leaveType)" effect="light" round>
              {{ getLeaveTypeText(row.leaveType) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="时间范围" min-width="300">
          <template #default="{ row }">
            <div class="date-range">
              <span class="date-text">{{ formatDateTime(row.startTime) }}</span>
              <el-icon class="mx-2" color="#909399"><Right /></el-icon>
              <span class="date-text">{{ formatDateTime(row.endTime) }}</span>
    ...
    <!-- 新建请假对话框 -->
    <el-dialog v-model="showDialog" title="新建请假申请" width="600px" destroy-on-close class="custom-dialog">
...
import { Plus, Search, Refresh, Document, Timer, CircleCheck, CircleClose, Right } from '@element-plus/icons-vue'
...
// Mock user ID - in real app get from store/token
const userId = ref(1)
...
const loadLeaveRequests = async () => {
  loading.value = true
  try {
    const response = await axios.get('/api/attendance/leave/my-requests', {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.post('/api/attendance/leave/apply', form.value)
    if (response.data.success) {
...
const filteredRequests = computed(() => {
  return leaveRequests.value.filter(item => {
    ...
    const matchType = !filterType.value || item.leaveType === filterType.value
...
const getLeaveTypeText = (type) => {
  const map = {
    'SICK': '病假', 'ANNUAL': '年假', 'PERSONAL': '事假',
    'MARRIAGE': '婚假', 'MATERNITY': '产假', 'PATERNITY': '陪产假', 'BEREAVEMENT': '丧假'
  }
  return map[type] || type
}

// Color coding for leave types
const getLeaveTypeTag = (type) => {
  const map = {
    'SICK': 'danger', 'ANNUAL': 'success', 'PERSONAL': 'warning',
    'MARRIAGE': 'danger', 'MATERNITY': 'success'
  }
  return map[type] || '' 
}
